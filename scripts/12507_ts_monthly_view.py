...
What it does: creates ts_reporting.v_monthly_entries, or updates it in
place when MONTHLY_VIEW_SQL below has changed. Every timetable row joined
to its user, task, project and client, ordered by employee then date. All
months - the month filter belongs to whoever reads the view, not here.

Schedule: monthly, via the schedule configured on this Data App in
Peliqan. Nothing in this script checks the date; a run just re-asserts
the view definition, so running it more often is harmless.
...
result = ensure_monthly_view()
say(f"{VIEW_SCHEMA}.{VIEW_NAME} created/updated.")
...
try:
    ...
    """) or []
    if counted:
        say(f"{counted[0].get('entries')} entries across "
            f"{counted[0].get('employees')} employees.")
except Exception as exc:
    # A failed count says nothing about whether the view itself is fine.
    say(f"View updated, but counting its rows failed: {exc}")

if INTERACTIVE and "st" in globals():
    st.caption("This app is not published - it maintains the view that "
               "ts_my_week's Excel export reads.")
    st.write(result)
